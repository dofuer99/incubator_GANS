...
class GAN():
    def __init__(self):
        ...
        self.pair_row = 24
        self.pair_column = 3
        self.img_rows = self.pair_row
        self.img_cols = 8 * self.pair_column
        self.channels = 1
    ...
    def train(self, epochs, batch_size=128, save_interval=50):
        ...
        #gourp together to create the new sample (number * 64 * 64 * 1 * 1)
        self.new_length = len(scaled) // (self.pair_row * self.pair_column)
        dropped_scaled = scaled[ : self.new_length * self.pair_row * self.pair_column]
        new_scaled = dropped_scaled.reshape((self.new_length, self.pair_row, self.pair_row))
...
#            final_fake_data = np.expand_dims(final_fake_data, axis=3)
    ...
    def save_data(self, epoch):
        gen_size = 15     #the number of fake data: 15 * 24 * 3
        ...
        gen_data_temp = gen_data.reshape(gen_size * self.pair_row * self.pair_column, -1)
        #unscale the data
    ...
    def save_imgs(self, epoch):
        ...
        #print (gen_imgs.shape)  (4, 24, 24, 1)
        ...
        for i in range(r):
            for j in range(c):
                imgs_plot = gen_imgs[ i * r + j]
                #rescale to -1, 1
                imgs_plot = 0.5 * imgs_plot + 0.5
                axs[i,j].imshow(imgs_plot[:,:,0], cmap='gray')
                axs[i,j].axis('off')
        fig.savefig("gen_data/images_%d.png" % epoch)
        plt.close()
        

if __name__ == '__main__':
    gan = GAN()
    gan.train(epochs=20001, batch_size=32, save_interval=1000)
