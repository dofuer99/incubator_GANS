...
#        input_data = np.array(test_new)        
        ...
        for epoch in range(epochs):
            ...
            real_train_acc = d_loss_real[1] * 100
            fake_train_acc = d_loss_fake[1] * 100
...
#            final_fake_data = np.expand_dims(final_fake_data, axis=3)
            ...
            #adaptive way
            if fake_acc > 0.7:
                #this indicates that our discriminator is very good
                self.n_critic = 1
                self.n_gen = 2
            elif fake_acc < 0.3:
                self.n_gen = 1
            ...
            # Plot the progress
            print ("%d [D loss: %f] [G loss: %f] [Real Acc: %.2f%%][Fake Acc: %.2f%%]" % (epoch, d_loss[0], g_loss, 100*real_acc, 100*fake_acc))
            
            if epoch % save_interval == 0:
                self.save_data(epoch)
                
            if epoch % 10 == 0 and epoch > 1:
                results_history.append([epoch, d_loss[0], g_loss, 100*real_acc, 100*fake_acc, 
                                        real_train_acc, fake_train_acc])
            ...
            if epoch == epochs - 1:
                ...
                G_loss = [row[2] for row in results_history]
                Fake_acc = [row[4] for row in results_history] 
                
                lns1 = ax.plot(xxx, Fake_acc, 'g', label='fake accuracy')
                ax2 = ax.twinx()
                ...
                ax.set_ylabel('Fake Acurracy (100%)')
                ax2.set_ylabel('Loss')
                ...
                Real_acc = [row[3] for row in results_history]
                Real_train_acc = [row[5] for row in results_history]
                Fake_train_acc = [row[6] for row in results_history]
                                
                fig1 = plt.figure()
                ax3 = fig1.add_subplot(111)    
                lns4 = ax3.plot(xxx, Real_acc, 'r', label='Test real data accuracy')
                lns5 = ax3.plot(xxx, Fake_acc, 'g', label='Test fake data accuracy')
                
                lns = lns4 + lns5 
                labs = [l.get_label() for l in lns]
                ax3.legend(lns,labs,loc=0)

                ax3.set_ylabel('Acurracy (100%)')
                ax3.set_xlabel('Iterations')
                ax3.set_ylim(-5,150)
                fig1.savefig('plot_test_accuracy')
 
                fig2 = plt.figure()
                ax4 = fig2.add_subplot(111)  
               
                lns6 = ax4.plot(xxx, Real_train_acc, 'r', label='Train real data accuracy')
                lns7 = ax4.plot(xxx, Fake_train_acc, 'g', label='Train fake data accuracy')
                
                lns = lns6 + lns7
                labs = [l.get_label() for l in lns]
                ax4.legend(lns,labs,loc=0)
                
                ax4.set_ylabel('Acurracy (100%)')
                ax4.set_xlabel('Iterations')
                ax4.set_ylim(-5,150)
                fig2.savefig('plot_train_accuracy')
...
if __name__ == '__main__':
    gan = GAN()
    gan.train(epochs=1201, batch_size=128, save_interval=100)
