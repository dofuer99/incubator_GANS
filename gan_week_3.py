...
#            final_fake_data = np.expand_dims(final_fake_data, axis=3)
            ...
            if epoch == epochs - 1:
                ...
                lns1 = ax.plot(xxx, D_accuracy, 'g', label='discriminative accuracy')
                ax2 = ax.twinx()
                ...
                ax3.plot(xxx, P_acc, 'g', label='test accuracy')
    ...
    def save_data(self, epoch):
        ...
        #unscale the data
        try3 = data_transform.unscaled(gen_data_new, self.cache) 
        unscaled = try3.unscale_begin()
        #round the data
        try4 = data_transform.round_data(unscaled)
        rounded_data = try4.round_all()
...
if __name__ == '__main__':
    gan = GAN()
    gan.train(epochs=3001, batch_size=128, save_interval=100)
